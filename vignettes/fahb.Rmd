...
We can use `print()` and `plot()` methods to summarise results, although note that the posterior predictive samples and the `brmsfit` model are both held within the analysis object so the user can summarise results in other ways if they please.

```{r}
print.fahb_analysis(analysis)
plot.fahb_analysis(analysis)
```

## External pilots

To do - implement extension to the external pilot setting.
