...
Our illustrative example suggests that standard recruitment PCs can be near optimal. They can also be found quickly (constrasting with the Bayesian approach which requires nested MCMC or an approximation), so we might conclude that these should be used. We therefore want to examine if this general near optimality holds across a broad range of scenarios. As an aside, we'd also like to understand if the kind of redundancy we've seen in the example is common.

We will conduct a simulation study. A scenario will be defined by: a prior; a feasibility threshold (which we will specify in terms of time to target recruitment); a target sample size; and a timing of interim analysis (as a proportion of the expected time to target recruitment). Within each scneario we will run the full Bayes analysis, and from this we will get the optimal FNR at a range of nominal FPRs. We will then look at each FPR and find the best FNR possible through standard PCs. The contrast of these FNRs will be our key metric.
