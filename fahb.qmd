...
We fit this model in a Bayesian framework. Given an appropriate joint prior for the model parameters (see below), we can take the fitted model and produce an estimated posterior predictive distribution of the final sample size in the trial, or, equivalently, of the final power.

To illustrate our approach, we can simulate some data from a trial based on the GUSTO example, recording both the final sample size and the internal pilot data. Note that we cap the total recruitment at the GUSTO target of 320 participants, and recruit for three years.
...
To choose the priors we can simulate the true recruitment rates they imply and check these are realistic. Recall that for GUSTO, the aim (for one of the gene expression subtypes) is to recruit 60 patients, randomise them, and then do a single arm analysis in the treatment arm. They anticipated recruiting 13 patients per month when all 20 centres are open - so an overall rate of 7.8 eligible patients per site per year.
...
We fit a Bayesian model to the pilot data using `brms`. The best way to do this is to first set up an empty model, which we can then call any time we have data to analyse. Note that our analysis model includes the same prior that we used to simulate the data - the relative merits of using a different prior at analysis, and in particular a less informative prior, will be discussed later.
...
sim_decision <- function(int_t){
  ...
  m_p <- nrow(int_data)
  rate <- 0
  if(sum(int_data$t) > 0) rate <- n_p / sum(int_data$t)
...
```{r, eval = FALSE}
ptm <- proc.time()
r <- t(replicate(10^3, sim_decision(int_t = 0.5)))
proc.time() - ptm

r <- as.data.frame(r)
proc.time() - ptm
names(r) <- c("mean_n", "m_p", "n_p", "rate", "end_n")
...
Now plot the data in the pilot sample space, flagging first if it is feasible (i.e. has an eventual power greater than 0.65) and then flagging the decision made when applying the Bayesian decision rule:

```{r}
df <- readRDS("data/test_r_6.rds")

thr <- 200

# Classify final sample sizes as feasible or not
df$feas <- df$end_n > thr
...
get_ocs <- function(thr, df){
  ...
  fpr <- sum(go[!df$feas])/sum(!df$feas)
  fnr <- sum(!go[df$feas])/sum(df$feas)
  
  #sens <- 1 - sum(go[df$feas])/sum(df$feas)
  #spec <- 1 - sum(!go[!df$feas])/sum(!df$feas)
  #return(c(sens, spec))
  return(c(fpr, fnr))
}
...
names(ocs)[2:3] <- c("fpr", "fnr") 

ggplot(ocs, aes(fpr, fnr, colour = thr)) + geom_line() +
...
ocs2 <- data.frame(fpr = opt$value[,1],
                   fnr = opt$value[,2],
                   t = "PC")

ocs2 <- rbind(ocs2, ocs[,2:4])

ggplot(ocs2, aes(fpr, fnr, linetype = t)) + geom_line() +
  xlab("False positive rate") + ylab("False negative rate") + 
  geom_point(data = ocs[ocs$thr == thr,]) +
...
So we see that, in this example at least, standard progression criteria are dominated by the Bayesian pre-specified decision rule in terms of sensitivity and specificity - though there is not a lot in it. We can look at the Pareto set of progression criteria:
...
To understand if it's worth trying to eye ball it, we could take one of these efficient rules and see how sens and spec change as we deviate from the optimal thresholds:
...
all_pc <- NULL
for(i in 1:10^3){
  pc_sens <- pc*runif(3, 0.5, 2)
  all_pc <- rbind(all_pc, pc_sens)
  sens_analysis <- rbind(sens_analysis, -get_pc_ocs(pcs = pc_sens, df = df))
}
plot(sens_analysis)
```
...
ggplot(df[df$n_p < 500,], aes(m_p, n_p, colour = go)) + geom_jitter(alpha = 0.3) +
  geom_hline(yintercept = floor(opt$par[2])) +
  geom_vline(xintercept = floor(opt$par[1])) + 
...
Note that the rate component rarely bites here:

```{r}
nrow(df[df$m_p > floor(opt$par[1]) & df$n_p > floor(opt$par[2]) & df$rate <= opt$par[3],])
```
That is, in our sample of 1000 we only have one case which passes the site number and participant number PC but then fails due to the rate PC. So this PC is also negligible (alongside the site PC), and we effectively have an "optimal" PC with a single component - proceed if we 5 or more participants recruited at 6 months. The OCs of this rule are:

```{r}
get_pc_ocs(c(0,4,0), df)
```
We plot this in our earlier OC graph as a single point. Finally, we can look at our motivating example. The progression criteria there are to stop if < 3 sites are open AND if <= 10 participants are recruited - note that this has a different form to the rules above, which specified that all criteria must be met to proceed.
...
Our illustrative example suggests that standard recruitment PCs can be near optimal. They can also be found quickly (contrasting with the Bayesian approach which requires nested MCMC or an approximation), so we might conclude that these should be used. We therefore want to examine if this general near optimality holds across a broad range of scenarios. As an aside, we'd also like to understand if the kind of redundancy we've seen in the example is common.

We will conduct a simulation study. A scenario will be defined by: a prior; a feasibility threshold (which we will specify in terms of time to target recruitment); a target sample size; and a timing of interim analysis (as a proportion of the expected time to target recruitment). Within each scenario we will run the full Bayes analysis, and from this we will get the optimal FNR at a range of nominal FPRs. We will then look at each FPR and find the best FNR possible through standard PCs. The contrast of these FNRs will be our key metric.

First, a new version of the simulation to allow for recruiting until the target sample size and returning the recruitment time:

```{r}
sim_rec_setup_eval <- function(m, int_t, target_n, 
  ...
  # First, get the expected recruitment 
  
  end_rec <- 10 # Length of recruitment period in years
  ...
  rec_rates <- cbind(rec_rates, c(0, rpois(m, (rec_rates[,3]*(rec_rates[,2] - rec_rates[,1]))[2:(m+1)])))
  ...
  # Now add n from previous periods and simulate interim period arrival times
  n_p <- sum(rec_rates[1:(int_period-1), 4]) + 
    sum(runif(rec_rates[int_period, 4], rec_rates[int_period, 1], rec_rates[int_period, 2]) < int_t)
  # Distribute these participants to sites in proportion to their expected numbers
  ...
  # Output the actual interim data and the final period
  # and some statistics of the pilot data
  return(list(data = df, rec_time = rec_time))
}

# For example,
sim <- sim_rec_setup_eval(m, int_t, target_n = 320, 
                   beta_m, beta_s, v_df, v_sc, setup_r_a, setup_r_b) 
```

Now, a routine to get the posterior expected recruitment time, given some simulated interim data:

```{r}
sim_end_time <- function(site_matrix, m, m_p, n_p, target_n){
  end_rec <- 1000
  
  # Site matrix is built below. Each row is a posterior sample. The columns
  # are the true site rates and the true site setup times, where 0 = end of interim
  us <- site_matrix[1:m]; setup_ts <- site_matrix[(m+1):(2*m)]
  # re-order
  ...
  # Simulate numbers recruited in each period
  rec_rates <- cbind(rec_rates, c(0, rpois(m, (rec_rates[,3]*(rec_rates[,2] - rec_rates[,1]))[2:(m+1)])))
  # Get time at which total target n is hit
  fin_period <- which(cumsum(rec_rates[,4]) > target_n)[1]
  n_needed <- target_n - sum(rec_rates[1:(fin_period-1), 4])
...
interim_eval <- function(sim){
  ...
  if(m - m_p > 0){
    ...
    # setup_times for sites not in the pilot
    setup_times <- t(sapply(setup_rates, function(x) cumsum(rexp(m-m_p, x))))
    ...
    rec_times <- apply(site_matrix, 1, sim_end_time, 
                       m=m, m_p=m_p, n_p=n_p, target_n=target_n)
  }
...
Now we want to repeatedly simulate under a given scenario and return the Bayesian decision and the three components to be used in a standard PC analysis:

```{r}
scenario_sim <- function(m, int_t, target_n, 
                         beta_m, beta_s, v_df, v_sc, setup_r_a, setup_r_b){
  
  sim <- sim_rec_setup_eval(m, int_t, target_n,
                   beta_m, beta_s, v_df, v_sc, setup_r_a, setup_r_b) 
  post_rec_time <- interim_eval(sim)
  n_p <- sum(sim$data$y)
  m_p <- nrow(sim$data)
  rate <- n_p/m_p
  
  return(c(n_p, m_p, rate, post_rec_time, sim$rec_time))
}

df <- t(replicate(5, scenario_sim(m, int_t, target_n, 
             beta_m, beta_s, v_df, v_sc, setup_r_a, setup_r_b)))
```
...
We could illustrate this easily. We could also explore a backwards approach where we ask what kind of adjustment would be necessary to bump the trial up into feasible territory - then, rather than eliciting a distribution for the effect, we only need to argue it will lie in a certain space. Further extension could look at extending the pilot trial to evaluate the adjustment - how much more data would we need to be sure it has worked?
...
Our framework also lets us examine decision rules which take for form of standard progression criteria and to optimise these if, for example, a funder requires rules in that format. And in our example, we found that these types of decision rules can be quite close to optimal, though potentially with some redundancy.
...
Note that SVMs in particular might help with the problem of uninformative priors leading to a very wide distribution of the data, since the focus will be on the middle. But also, centering and normalising the data features could fix that for the GAM approach.
