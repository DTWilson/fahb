...
sim_rec2 <- function(m, m_p, n_p, mu, sig) {
  # Instead of stopping at n_p, stop at a fixed time
...
# Get posterior distribution of utility (= power of main trial, conditional on 
# overall rate lambda but averaging over numbers actually recruited)
us <- sapply(lambda, cond_exp_pow) - 0.5

# Expected utility = expected power above threshold of 0.5. Proceed if exp(u) > 0.
mean(us)
```
...
Now consider a situation as in ARIEL, where sites will start at a random time. The pilot design is therefore not the actual number of sites, but the time of analysis.

```{r}
m <- 40
t_p <- 1 # time we'll do the analysis

# alternative
pars <- ln_pars(m = 3.3, s = 3)
...
sim_rec2 <- function(m, t_p, mu, sig) {
  # Instead of stopping at n_p, stop at a fixed time t_p
  
  # True recruitment rates
  lambdas <- exp(rnorm(m, mu, sig))
  
  # Site start times
  starts <- cumsum(rexp(m, m/3))
  starts <- starts[starts < t_p]
  
  m_p <- length(starts)
  
  # Wait times at each pilot site
  t <- matrix(rexp(m_p*200, rate = lambdas[1:m_p]), ncol = m_p, byrow = T)
  # Transform to arrival times
  t2 <- apply(t, 2, cumsum)
  # Add start time delays
  t2 <- t(t(t2) + starts)
  # Merged arrival times
  t3 <- melt(t2)[,2:3]
  t3 <- t3[order(t3$value),]
  ...
  return(list(data = df, lambdas = lambdas, 
              stats = c(log(mean(ns/time)), log(sd(ns/time)))))
}

pilot_sim <- sim_rec2(m, t_p, mu, sig)
```
...
fit  <- brm(y ~ 1 + offset(log(t)) + (1 | c), data = pilot_sim$data, family = poisson(),
            prior = bprior, silent = 2)
...
rs <- NULL
  ...
  for(t_p in t_ps){
    ...
    lambda <- rowSums(us)/t_p
    mean(lambda > 1250)
    
    r <- c(t_p, m_p, sum(pilot_sim$data$y),
           sum(pilot_sim$lambdas),
           mean(lambda),
           sd(lambda),
           as.numeric(quantile(lambda, probs = c(0.05, 0.95))))
...
results <- as.data.frame(rs)
names(results) <- c("t_p", "n_p", "true_n_p", "true", "est", "sd", "l_cr", "u_cr")
```

```{r}
results$rel_cr <- (results$u_cr - results$l_cr)/results$true

ggplot(results, aes(x = t_p, group = t_p)) +
  geom_boxplot(aes(y=u_cr - l_cr)) + ylim(c(0, 1000))

ggplot(results, aes(x = true_n_p, colour = t_p)) +
  geom_point(aes(y=u_cr - l_cr)) + ylim(c(0, 1000))

ggplot(results, aes(x = n_p, colour = t_p)) +
  geom_point(aes(y=u_cr - l_cr)) + ylim(c(0, 1000))
```
...
for(i in 1:10^4){
  # A more informative design prior
  mu <- rnorm(1, 3.6, 0.5)
  sig <- rgamma(1, shape = 3, rate = 6) #rht(1, nu = 50, sigma = 0.3)
  #sig <- rht(1, nu = 50, sigma = 0.3)
  ...
  pilot_sim <- sim_rec2(m, m_p, n_p, mu, sig)
  r <- c(cond_exp_pow(sum(pilot_sim$lambdas)) - 0.75, pilot_sim$stats, sum(pilot_sim$lambdas))
  df2 <- rbind(df2, r)
}
names(df2) <- c("u", "m", "sd", "l")

r <- data.frame(u = df2$u,
                p_u = predict(fit, newdata = df2[,2:3]))
...
mean(r[r$u < 0,]$p_u > 0, na.rm = T)
mean(r[r$u > 0,]$p_u < 0, na.rm = T)

mean(r[r$p_u < 0,]$u > 0, na.rm = T)
mean(r[r$p_u > 0,]$u < 0, na.rm = T)

# Probability of a correct decision
mean((r$u > 0 & r$p_u > 0) | (r$u < 0 & r$p_u < 0))

r$go <- r$p_u > 0

ggplot(r, aes(u+0.75, colour = go)) + geom_density(aes(y = after_stat(count))) +
  geom_vline(xintercept = 0.75, linetype = 2)
```

We can also compare this against a simple PC approach - for any given PC, we can find a correpsonding decision and then plot the same OCs.
...
for(i in 1:10^3){
  ...
  x <- c(cond_exp_pow(sum(pilot_sim$lambdas)) - 0.75, pilot_sim$stats)
  df <- rbind(df, x)
}
names(df) <- c("u", "m", "sd")

plot(predict(fit, newdata = df[,2:3]), df$u)

df$p_u <- predict(fit, newdata = df[,2:3])
...
ggplot(r, aes(u, colour = go)) + geom_density(aes(y = after_stat(count)))

# Repeat for alternative hypothesis

# alternative, matching moments of gamma with sh = 3
pars <- ln_pars(m = 50, s = 29)
```


```{r}
X <- matrix(c(rep(1, 80), rep(0:1, each = 40)), ncol = 2)
sig <- 1
solve(t(X) %*% X)
...
Can we motivate Bayesian approach, partially, via the likelihood principle?
...
# Fixed time, recruited n
1 - pnbinom(n-1, alpha*m, beta/(beta+1))

# Sum of n exponentials (waiting times) with rate beta is gamma(n, beta)
pgamma(1, n, beta)
