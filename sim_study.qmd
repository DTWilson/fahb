...
plot(full_res[full_res[,1] == 50, c(2,3)])
points(full_res[full_res[,1] == 50, c(2,7)], col="red")
```
...
res <- full_res[full_res[,2] == 0.20,]

res <- as.data.frame(cbind(res[,c(2,3,7)], ext_design))
names(res)[2:3] <- c("PC_fnr", "Bayes_fnr")

ggplot(res, aes(m_p, target_n_p, fill=Bayes_fnr)) + geom_tile() +
